...
theorem mmatrix_def (M : Matrix ι ι ℝ) : MMatrix M ↔ ∃ B, ∃ r, (∀ i j, 0 ≤ B i j) ∧ ‖B‖ < r ∧ M = r • 1 - B := by
  apply Iff.intro
  · intro h
    let r := Real.toNNReal <| (Finset.univ.image (fun i => |M i i|)).max' (Finset.image_nonempty.mpr (Finset.univ_nonempty))
    let B := r • 1 - M
    use B
    use r
    have Bpos (i j) : 0 ≤ B i j := by
      simp [B]
      by_cases h' : i = j
      · rw [h', Matrix.one_apply_eq, NNReal.smul_def, smul_eq_mul, mul_one]
        by_contra!
        simp [r] at this
        apply And.left at this
        specialize this j
        have : False :=
          ne_of_lt (abs_lt.mp this).right <| Eq.refl _
        contradiction
      · rw [Matrix.one_apply_ne h', smul_zero]
        exact h.nonpos_off_diag i j h'
    have Meq : M = r • 1 - B := by
      simp [B]
    refine ⟨Bpos, ?_, Meq⟩
  ...
  · intro ⟨B, r, hpos, hle, heq⟩
    ...
    · intro i j
      ...
      have smbl : Summable (HPow.hPow (r⁻¹ • B)) := by exact
        HasSummableGeomSeries.summable_geometric_of_norm_lt_one (r⁻¹ • B) Brspec
